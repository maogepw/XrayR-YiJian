...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/maogepw/XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 XrayR 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 XrayR 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 XrayR 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux-64.zip https://github.com/maogepw/XrayR/releases/download/${last_version}/XrayR-linux-64.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/maogepw/XrayR/releases/download/${last_version}/XrayR-linux-64.zip"
    ...
    file="https://github.com/maogepw/XrayR-V2Board/raw/main/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "全新安装，请先参看教程：https://github.com/maogepw/XrayR，配置必要的内容"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR 可能启动失败，请稍后使用 XrayR log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/maogepw/XrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/maogepw/XrayR-script/master/XrayR.sh
    ...
    # Writing json
    echo "正在尝试写入配置文件..."
    #wget https://cdn.jsdelivr.net/gh/missuo/XrayR-YiJian/config.yml -O /etc/XrayR/config.yml
    curl -o /etc/XrayR/config.yml -Ls https://cdn.jsdelivr.net/gh/missuo/XrayR-YiJian/config.yml
    ...
    echo "Telegram: https://t.me/maoge521"
    echo "Github: https://github.com/maogepw/XrayR-YiJian"
    echo "Powered by 猫哥"
}
